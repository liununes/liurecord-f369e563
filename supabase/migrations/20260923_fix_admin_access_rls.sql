...
DROP POLICY IF EXISTS "Public read site_theme" ON public.site_theme;
...
-- 4) clients / client_photos (se existirem): somente service_role (edge functions).
DO $$
BEGIN
  IF to_regclass('public.clients') IS NOT NULL THEN
    EXECUTE 'DROP POLICY IF EXISTS "Allow all on clients" ON public.clients';
  ...
  IF to_regclass('public.client_photos') IS NOT NULL THEN
    EXECUTE 'DROP POLICY IF EXISTS "Allow all on client_photos" ON public.client_photos';
    EXECUTE 'REVOKE ALL ON public.client_photos FROM anon, authenticated';
    EXECUTE 'GRANT ALL ON public.client_photos TO service_role';
  END IF;
END $$;
