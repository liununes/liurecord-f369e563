...
const PortfolioSection = () => {
  ...
  const { data: audioItems } = usePortfolioMedia("audio");
  ...
  const items = audioItems && audioItems.length > 0 ? audioItems : fallbackItems;
    ...
    <section id="portfolio" className="py-24">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <h2 className="font-display text-5xl md:text-6xl tracking-wider text-foreground">Portfólio de Áudios</h2>
        ...
        <div className="max-w-2xl mx-auto space-y-4">
          {items.map((sample: any, index: number) => (
            <div key={index} className="flex items-center gap-4 bg-card border border-border rounded-lg p-5 hover:border-primary/40 transition-all duration-300 group cursor-pointer" onClick={() => togglePlay(index)}>
              <button className="flex-shrink-0 w-12 h-12 rounded-full bg-gradient-gold flex items-center justify-center text-primary-foreground hover:opacity-90 transition-opacity" aria-label={playingIndex === index ? "Pausar" : "Reproduzir"}>
              ...
              <div className="hidden sm:flex items-end gap-0.5 h-8">
                {Array.from({ length: 20 }).map((_, i) => (
                  <div key={i} className={`w-1 rounded-full transition-colors duration-300 ${playingIndex === index ? "bg-primary" : "bg-muted-foreground/30"}`} style={{ height: `${Math.random() * 100}%`, minHeight: "4px" }} />
                ))}
              </div>
            </div>
          ))}
